...
;
IF OBJECT_ID('tempdb..#naEBITDA') IS NOT NULL
	DROP TABLE #naEBITDA
GO
...
SELECT dp.Country, dp.Entity, dp.Market, dp.Region, dp.State, dp.Actual_Building, dpi.Post_Month, SUM(yfd.[Actual]) as [EBITDA]
INTO #naEBITDA
...
WHERE yfd.Account_Tree_Node_Description = 'EBITDA'
...
SELECT DISTINCT Entity, Actual_Building,
	SUM([EBITDA]) OVER (PARTITION BY Actual_Building)/1000000 AS EBITDA
FROM #naEBITDA
