...
;
IF OBJECT_ID('tempdb..#naREVENUE') IS NOT NULL
	DROP TABLE #naREVENUE
...
SELECT dp.Country, dp.Entity, dp.Market, dp.Region, dp.State, dp.Actual_Building, dpi.Post_Month, SUM(yfd.[Actual]) as [REVENUE]
INTO #naREVENUE
...
WHERE yfd.Account_Tree_Node_Description = 'TOTAL REVENUES'
...
SELECT DISTINCT Entity, Actual_Building,
	SUM([REVENUE]) OVER (PARTITION BY Actual_Building)/1000000 AS REVENUE
FROM #naREVENUE
WHERE Post_Month BETWEEN '2024-01-01' AND '2024-03-01'